...
function PersonCard({ title, Icon, actionLabel, draft, setDraft, people, setPeople, saving }: {
  ...
  return <section className="unit-reference-card unit-reference-person-card">
    ...
    <div className="unit-reference-person-inputs">
      <Field id={`${title}-name`} label="Nama lengkap" placeholder="Masukkan nama lengkap" value={draft.name} onChange={(event) => setDraft((current) => ({ ...current, name: event.target.value }))} required={false} disabled={saving} />
...
function UnitCreateForm({ unit, relatedAdmins = [], saving, error, setError, submit }: { unit?: Unit; relatedAdmins?: RelatedAdmin[]; saving: boolean; error: string; setError: React.Dispatch<React.SetStateAction<string>>; submit: (event: React.FormEvent<HTMLFormElement>) => Promise<void> }) {
  ...
  async function removeSavedAdmin(admin: RelatedAdmin) {
    ...
    try {
      ...
      setError(getUserFacingMessage(reason, 'Akun admin unit belum bisa dihapus. Periksa data akun lalu coba lagi.'));
    }
    finally {
      setDeletingAdminId('');
    }
  ...
  return <TooltipProvider delayDuration={120}><form onSubmit={onSubmit} className="unit-reference-form">
    ...
    <section className="unit-reference-card unit-reference-admin-card">
      ...
      <div className="unit-reference-admin-inputs">
        <Field id="admin-name" label="Nama admin unit" placeholder="Masukkan nama admin unit" value={adminDraft.name} onChange={(event) => setAdminDraft((current) => ({ ...current, name: event.target.value }))} required={false} disabled={saving} />
        <Field id="admin-email" label="Email akun" type="email" placeholder="Masukkan email akun" value={adminDraft.email} onChange={(event) => setAdminDraft((current) => ({ ...current, email: event.target.value }))} required={false} disabled={saving} />
    ...
    <footer className="unit-reference-footer"><Button asChild type="button" variant="outline" className="unit-reference-cancel" aria-disabled={saving}><Link href="/unit-management">Batal</Link></Button><Button type="submit" className="unit-reference-save" disabled={saving}>{saving ? <Loader2 className="animate-spin" /> : <Save />}{saving ? 'Menyimpan...' : unit ? 'Simpan perubahan' : 'Simpan unit'}</Button></footer>
