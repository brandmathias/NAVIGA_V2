...
import { INDONESIAN_PROVINCES, formatUnitCode } from '@/lib/unit-code';
...
export default function UnitCreateClient({ units, mode }: { units: Unit[]; mode: 'unit' | 'admin' }) {
  ...
  return <main className="unit-create-page flex flex-1 flex-col gap-5 px-4 py-5 sm:px-6 lg:px-9">
    <header className="unit-create-heading"><button type="button" onClick={() => router.push('/unit-management')} className="unit-create-back"><ChevronLeft className="h-4 w-4" /> Manajemen Unit</button><div className="mt-3 flex items-center gap-4"><span className="unit-admin-dialog-emblem"><ShieldPlus className="h-9 w-9" strokeWidth={1.55} /></span><div><p className="text-sm font-semibold text-primary">{mode === 'unit' ? 'Manajemen Unit / Tambah Unit' : 'Manajemen Unit / Tambah Akun'}</p><h1 className="font-headline text-3xl font-bold tracking-[-.04em] text-[#064c55] sm:text-4xl">{mode === 'unit' ? 'Tambah Unit' : 'Tambah akun admin unit'}</h1><p className="mt-1 text-sm text-[#607786]">{mode === 'unit' ? 'Lengkapi informasi unit dan admin awal untuk menambah data ke sistem.' : 'Buat akun admin baru untuk mengelola unit terkait.'}</p></div></div></header>

    <form onSubmit={submit} className="unit-create-card">
      ...
      {mode === 'unit' ? <>
        <section><div className="unit-create-section-title"><Building2 className="h-5 w-5" /> <h2>Informasi Unit</h2></div><div className="mt-5 grid gap-4 lg:grid-cols-3">
          ...
          <div className="unit-admin-field"><span className="unit-admin-field-icon"><MapPin className="h-5 w-5" strokeWidth={1.8} /></span><div className="min-w-0"><Label className="unit-admin-field-label">Provinsi <span>*</span></Label><Select name="province" value={province} onValueChange={setProvince} required disabled={saving}><SelectTrigger className="unit-admin-select-trigger mt-2"><SelectValue placeholder="Pilih provinsi" /></SelectTrigger><SelectContent className="unit-admin-select-content">{INDONESIAN_PROVINCES.map((item) => <SelectItem key={item} value={item} className="unit-admin-select-item">{item}</SelectItem>)}</SelectContent></Select></div></div>
      ...
      <div className="unit-admin-actions mt-6"><Button type="button" variant="outline" className="unit-admin-cancel" onClick={() => router.push('/unit-management')} disabled={saving}>Batalkan</Button><Button type="submit" className="unit-admin-submit" disabled={saving || (mode === 'admin' && !unitId)}>{saving ? <Loader2 className="h-5 w-5 animate-spin" /> : <Save className="h-5 w-5" />}{saving ? 'Menyimpan...' : mode === 'unit' ? 'Simpan unit' : 'Simpan akun'}</Button></div>
    </form>
  </main>;
}
