...
test('unit management matches the operational hierarchy and keeps complete unit and admin fields reachable', async () => {
  ...
  assert.match(styles, /\.naviga-topbar\s*\{[\s\S]*?background:\s*rgba\(255, 255, 255, \.96\)/);
  assert.match(styles, /\.naviga-topbar\[data-scrolled="true"\]\s*\{[\s\S]*?background:\s*rgba\(255, 255, 255, \.62\)/);
  assert.match(styles, /backdrop-filter:\s*blur\(10px\) saturate\(1\.04\)/);
  ...
  assert.match(shell, /const \[isScrolled, setIsScrolled\] = React\.useState\(false\)/);
  assert.match(shell, /window\.addEventListener\('scroll', updateScroll, \{ passive: true \}\)/);
