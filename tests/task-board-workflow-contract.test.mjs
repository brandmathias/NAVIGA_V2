...
test('task board UI keeps the star action, aligned priority visuals, and compact cards', () => {
  ...
  assert.match(board, /const canDeleteColumn = !isReadOnlyView && boardData\.columnOrder\.length > 1/);
  ...
  assert.match(board, /hover:-translate-y-0\.5/);
  assert.match(board, /active:translate-y-0/);
  assert.match(board, /active:scale-95/);
  assert.match(board, /focus-visible:ring-2/);
  assert.match(board, /pendingColumnDeletion/);
  assert.match(board, /setPendingColumnDeletion/);
  assert.match(board, /confirmDeleteColumn/);
  assert.match(board, /Hapus kolom\?/);
  assert.match(board, /Kolom berisi/);
  assert.doesNotMatch(board, /tugas tetap tersimpan/);
  assert.doesNotMatch(board, /Tugas dalam kolom ini akan dipindahkan/);
  assert.doesNotMatch(board, /bg-\[#fff1f2\]/);
  assert.doesNotMatch(board, /<Trash2 className="h-5 w-5"/);
  assert.match(board, /DialogFooter className="[^\"]*justify-between/);
  assert.match(board, /DialogFooter className="[^\"]*sm:justify-between/);
  assert.match(board, /DialogFooter[\s\S]*?Batal[\s\S]*?Hapus kolom/);
  assert.match(board, /In Progress/);
  assert.match(board, /grid-flow-col/);
  assert.match(board, /overflow-x-auto/);
  assert.match(board, /auto-cols-\[minmax\(280px,1fr\)\]/);
  assert.match(board, /aria-label="Tambah kolom"/);
  assert.match(board, /onClick=\{submitNewColumn\}/);
  assert.doesNotMatch(page, /Tambah Kolom<\/Button>/);
  assert.match(page, /onAddColumn=\{handleAddColumn\}/);
  assert.match(page, /onDeleteColumn=\{handleDeleteColumn\}/);
  assert.match(page, /const remainingOrder = previous\.columnOrder\.filter/);
  assert.match(page, /taskIds: \[\.\.\.targetColumn\.taskIds, \.\.\.column\.taskIds\]/);
